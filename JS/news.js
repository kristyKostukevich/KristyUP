...
var Arraytags=['Политика', 'музыка', 'спорт','архитектура','минск','финансы','культура','метро'];
console.log("Hi! I'm js");

function getArtcls(skip, top ,filterConfig) {
    if (skip == undefined) skip = 10;
    top = top || 10;
    var flag = 0;
    ...
    if (filterConfig != undefined) {
        ...
        if (filterConfig.date != undefined) {
            rezaltArray = rezaltArray.filter(function (temp) {
                    if (temp.createdAt - filterConfig.date == 0)
                    return true;
                    else return false;
            });
        ...
        if (filterConfig.tags != undefined)
            rezaltArray = rezaltArray.filter(function (temp) {
                ...
                for (var j=0; j<filterConfig.tags.length; j++)
                    if(temp.tags[i]==filterConfig.tags[j])
                        count++;

                if (count == filterConfig.tags.length)
                    return true;
                else return false;
            });
    }
...
console.log("фильтрация по тегу");
   console.log(getArtcls(0, 3, filtr));
console.log("фильтрация по автору и дате");
console.log(getArtcls(0, 3, {author: "NickZabolotskiy",
    date: new Date('2016-04-12T21:42:05'),}));
...
function addArticle(article){
    ...
    if(validateArticle(article) == true&&getArticle(article.id)== undefined)
    {
        for (var i=0; i<article.tags.length; i++)

        {   var tag =article.tags[i];
            if (haveTag(tag)==false)
...
console.log(getArticle('11'));

console.log(editArticle('11', {
    title: 'Минское «Динамо» обыграло ярославский «Локомотив»',
    summary: 'Минское «Динамо» обыграло ярославский «Локомотив» в четвертом матче первого раунда плей-офф КХЛ — 4:2',
...
console.log(editArticle('33', {id: '11',
    title: 'Минское «Динамо» обыграло ярославский «Локомотив»',
    summary: 'Минское «Динамо» обыграло ярославский «Локомотив» в четвертом матче первого раунда плей-офф КХЛ — 4:2',
    createdAt: new Date('2017-02-27T23:00:00'),
    author: 'Иванов Иван',
